...
from settings import APIS, MYKTV_SECRET, SECURET, CAR_SECURET
...
def check_sign_ex(timstamp, sign):
    if timstamp == '1111':
        return True
    sign_str = ''.join(list(map(str, [SECURET, timstamp])))
    mysign = hashlib.md5(sign_str.encode()).hexdigest()
    if mysign == sign:
        return True

    sign_str = ''.join(list(map(str, [CAR_SECURET, timstamp])))
    mysign = hashlib.md5(sign_str.encode()).hexdigest()
    return mysign == sign
...
def check_sign(timstamp, ktv_id, sign):
    ...
    sign_str = ''.join(list(map(str, [CAR_SECURET, timstamp, ktv_id])))
    mysign = hashlib.md5(sign_str.encode()).hexdigest()
    return mysign == sign

async def _get_base64_img(username, ktv_id):
    try:
        response = await async_common_api('/wx/arged/qrcode', dict(flag=3, arg='%s' % ktv_id))
        qr = qrcode.QRCode(
            version=1,
        ...
        logging.error('ktv%s生成二维码失败，失败原因: %s' % (ktv_id, e))
        return None

def is_T1_KTV(ktv):
    if ktv and float(ktv.get('rate', '0.0')) != 0.0:
        return True
    return False

def calc_rate_fee(fee, rate):
    rate_fee_src = fee * rate
    rate_fee_1 = round(rate_fee_src, 1)
    rate_fee = round(rate_fee_src)
    if rate_fee_1 > rate_fee:
        rate_fee += 1
    return rate_fee
